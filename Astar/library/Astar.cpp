...
G cost := Path taked distance between CurrentPose(x,y,z) and BeginningPose(x,y,z)
F cost
*/
/* Default Constructor */
...
*/
void AstarContainer::Astar::shiftObjects() {
...
Finishes by making a node out of the cordinates
*/
void AstarContainer::Astar::shiftEndNode() {
...
void AstarContainer::Astar::enlargeObjects(int stepsOuts) {
...
void AstarContainer::Astar::addNode(std::vector<double> cordinates,Node& parent) {
    ...
    if (it != openNodes.end()) {
        if (CompareVectors(endPos,cordinates,.01)) {
        ...
        if (ObjectCheck(objectVector,cordinates,.01)) {
            double host = -1;
            Node node(cordinates[0],cordinates[1],cordinates[2],deadNode,host);
            openNodes[cordinates] = node;
        }
        else {
            double host = nodeHost(cordinates[0],cordinates[1],cordinates[2]);
            Node node(cordinates[0],cordinates[1],cordinates[2],parent,host);
            openNodes[cordinates] = node;
...
void AstarContainer::Astar::explore(Node& currentNode) {
    std::vector<double> V;
    V.clear();
    V.push_back(0);
    V.push_back(0);
...
void AstarContainer::Astar::addChoosenNodes(Node& node) { //,std::vector<double> pose) {
    pq.push(node);
}
...
void AstarContainer::Astar::finalizePath(Node& node_in_Path) {
    if (node_in_Path.parent->gost == -1) {
        ...
        Path.push(node_in_Path);
        std::cout << node_in_Path.parent->x << " " << node_in_Path.parent->y << " " << node_in_Path.parent->z << std::endl; //** being used to print values for plotting
...
void AstarContainer::Astar::displayOpenNodes() {
    ///*
